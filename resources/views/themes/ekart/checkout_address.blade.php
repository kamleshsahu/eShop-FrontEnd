    ...
    <div class="container">
        <div class="card shadow-sm mb-4">
            <div class="row">
                ...
                <div class="col-md-3 text-center">
                    <span class="icon dark"><em class="fa fa-chevron-circle-right delivery-icon"></em> Address</span>
                </div>
            ...
            <div class="row padding-bottom">
                <div class="col-md-12" id="address">
                    <div class="card shadow mb-4">
                        <p class="product-name pb-0 font-weight-bold head" id="mydesc">Delivery Address</p>
                        <hr class="mb-0">
                                        ...
                                        <div class="col-md-12">
                                            <form action="{{ route('checkout-address') }}" method="POST">
                                                @foreach($data['address'] as $a)
                                                    <div class="row delivery-address">
                                                        ...
                                                        <span class="form-group ml-2">
                                                            <input type="radio" name="id" value="{{ $a->id }}" {{ (isset($a->is_default) && intval($a->is_default) ? 'checked=checked' : '') }}>
                                                            <label>
                                                                <strong>{{ $a->name ?? '' }}</strong><br>
                                                                <label class="badge badge-primary">{{ $a->type }}</label> 
                                                                @if(isset($a->is_default) && intval($a->is_default))
                                                                    <label class="badge badge-primary">Default</label> 
                                                                @endif
                                                ...
                                                <div class="form-group mb-0 float-right">
                                                    <span onclick="address()" class="btn btn-primary text-uppercase">Add New Address</span>
                                                    <button type="submit" class="btn btn-primary text-uppercase add-to-cart">Confirm <em class="fa fa-check"></em></button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
...
$(document).ready(function(){
    ...
    @if(isset(session()->get('checkout-address', NULL)->id) && intval(session()->get('checkout-address', NULL)->id))
        $("input[type=radio][name=id][value={{ session()->get('checkout-address', NULL)->id ?? 0 }}]").prop("checked", "checked");
    @endif
});
</script>